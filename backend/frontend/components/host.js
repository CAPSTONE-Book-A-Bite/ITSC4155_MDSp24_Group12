window.onload = function () {
  ...
  document.querySelector(
    ".restaurant-text"
  ).innerHTML = `${hostName} Restaurant Booking List`;
  // get the host's id
  ...
  fetch("http://localhost:3001/api/reservations/", {
    ...
    .then((data) => {
      ...
      const reservations = data.reservations.filter(
        (reservation) => reservation.restaurant == hostName
      );
      ...
      const upcomingReservations = reservations.filter(
        (reservation) =>
          new Date(reservation.datetime) >= today &&
          new Date(reservation.datetime) <= nextWeek
      );
      ...
      document.querySelector(".underlined-reservations").innerHTML =
        upcomingReservations.length;
      // if there are no reservations for the week, display a message
      if (upcomingReservations.length == 0) {
        document.querySelector(".reservation-number").innerHTML =
          "There are no reservations for this upcoming week";
      }
      // if there is only 1 reservation for the week, display a message
      else if (upcomingReservations.length == 1) {
        document.querySelector(".reservation-number").innerHTML =
          "There is <u>1</u> reservation for this upcoming week";
      }
...
document
  .getElementById("reservations-table")
    ...
    if (event.target.className === "cancel-button") {
      ...
      try {
        ...
        if (table.rows.length == 1) {
          document.getElementById("reservation-number").innerHTML =
            "There are no reservations for this upcoming week";
          ...
          document.getElementById(
            "reservation-number"
          ).innerHTML = `There are <u class="underlined-reservations">${
            table.rows.length - 1
          }</u> reservations for this upcoming week`;
        }
      } catch (error) {
