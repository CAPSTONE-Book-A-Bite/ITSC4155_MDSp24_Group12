  ...
  await fetch("http://localhost:3001/api/reservations")
    ...
    .then((data) => {
      ...
      if (Array.isArray(reservations)) {
        ...
        reservations = reservations.filter(
          (reservation) => new Date(reservation.datetime) > new Date()
        );
        ...
        if (reservations.length > 0) {
          ...
          let reservationTime = new Date(
            reservations[0].datetime
          ).toLocaleTimeString();
          // remove seconds from time but keep AM/PM
          reservationTime =
            reservationTime.slice(0, -6) + reservationTime.slice(-3);
    ...
    .then((data) => {
      ...
      if (Array.isArray(restaurants)) {
        restaurants.forEach(async (restaurant) => {
          ...
          await fetch(
            `http://localhost:3001/api/admin/lastUpdated/${restaurant.name}`
          )
            .then((response) => {
              if (!response.ok) {
        ...
        try {
          ...
          if (document.getElementById("reservation-table").rows.length > 1) {
            ...
            const reservationTime =
              document.getElementById("reservation-table").rows[1].cells[2]
                .innerHTML;
            if (reservationDate > now) {
              noReservations.innerHTML = `Your next reservation is on ${reservationDate.toDateString()} at ${reservationTime}`;
            } else {
