window.onload = function () {
  ...
  fetch("http://localhost:3001/api/admin")
    ...
    .then((data) => {
      ...
      restaurant = resteraunts.find(
        (resteraunt) => resteraunt.name.replace(/\s/g, "") === restaurantName
      );

      const trimmedName = restaurant.name.replace(/\s/g, "");
      document.getElementById("restaurant-image").src =
        "/api/images/" + trimmedName;
  ...
  fetch("http://localhost:3001/api/users")
    ...
    .then((data) => {
      ...
      document.getElementById("your-phone").innerHTML =
  ...
  fetch(`http://localhost:3001/api/reservations/${reservationId}`)
    ...
    .then((data) => {
      ...
      document.getElementById(
        "reservation-created"
      ).innerHTML = `<b class="created">Reservation Confirmed!</b><br> Your Book-A-Bite Reservation ID </br>for <b>${reservation.restaurant}</b> is <b>${reservation.id}</b> <br>under the name <b>${reservation.name}</b></br> <br><b>Booking Date: ${date} ${time}</b></br>`;
    })
    .catch((error) => {
