...
const getReservationById = async (req, res, next) => {
  ...
  let reservation;
  try {
    reservation = await db.query("SELECT * FROM reservations WHERE id = $1;", [
      reservationId,
    ]);
...
const updateReservation = async (req, res, next) => {
  ...
  try {
    ...
    if (!reservation) {
      return res
        .status(404)
        .json({ message: "Could not find reservation for the provided id." });
    }
    ...
    return next(
      new HttpError("Something went wrong, could not update reservation.", 500)
    );
  }
};
...
const deleteReservation = async (req, res, next) => {
  ...
  try {
    ...
    if (reservation.rowCount == 0) {
      const error = new HttpError(
        "Could not find a reservation for this id.",
        404
      );
      return res.status(404).json({ error: error });
    }
